...
export const FaceTimeContent: React.FC = () => {
  ...
  const drawCatOverlay = (
    ...
    const leftEyeOpenness = Math.abs(
      (leftEyeLower.y - leftEyeUpper.y) * height,
    );
    const rightEyeOpenness = Math.abs(
      (rightEyeLower.y - rightEyeUpper.y) * height,
    );
    ...
    const eyeHeightRight = isRightEyeClosed
      ? faceHeight * 0.02
      : faceHeight * 0.1;
    ...
    for (let i = 0; i < 3; i++) {
      ...
      ctx.lineTo(
        -faceWidth * 0.7,
        y - faceHeight * 0.05 + i * (faceHeight * 0.04),
      );
      ctx.stroke();
    }
  ...
  const drawTalwinderOverlay = (
    ...
    const rightEyeOpenness = Math.abs(
      (rightEyeBottom.y - rightEyeTop.y) * height,
    );
    ...
    if (!isLeftEyeClosed) {
      ...
      ctx.beginPath();
      ctx.arc(
        -eyeOffsetX - eyeHeight * 0.2,
        -eyeOffsetY - eyeHeight * 0.2,
        eyeHeight * 0.25,
        0,
        Math.PI * 2,
      );
