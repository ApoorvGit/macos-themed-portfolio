...
export const StickyNote: React.FC<StickyNoteProps> = ({
  ...
  const handleTopBarMouseDown = (e: React.MouseEvent) => {
    ...
    if (target.closest('.sticky-button') || target.classList.contains('sticky-button')) {
      return;
    }
  ...
  const handleZoom = (e: React.MouseEvent) => {
    ...
    if (isZoomed) {
      ...
      const maxHeight = Math.min(targetHeight, window.innerHeight - position.y - 40);
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-1.5">
          ...
          >
            <svg width="10" height="10" viewBox="0 0 10 10" className="absolute top-0 left-0">
              <path
                d="M 0 10 L 10 10 L 10 0 Z"
                fill={colors.button.includes('yellow') ? 'rgba(251, 191, 36, 0.6)' :
                      colors.button.includes('pink') ? 'rgba(244, 114, 182, 0.6)' :
                      colors.button.includes('green') ? 'rgba(74, 222, 128, 0.6)' :
                      colors.button.includes('blue') ? 'rgba(96, 165, 250, 0.6)' :
      ...
      {/* Content */}
      {!isCollapsed && (
        <div className="p-4 overflow-auto" style={{ height: `calc(${size.height}px - 20px)` }}>
      ...
      {!isCollapsed && (
        ...
        >
          <svg
            ...
            fill="currentColor"
          >
            <path d="M14 14L14 10M14 14L10 14M14 14L8 8M14 6L6 14" stroke="currentColor" strokeWidth="1.5" />
          </svg>
        </div>
      )}
    </div>
